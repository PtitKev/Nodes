...
void ydle::onCommandReceived(Frame_t *frame)
{
  ...
  while (extractData(frame, index, litype, livalue) == 1)
  {
    ...
    if (litype == YDLE_CMD_LINK && !m_initializedState)
    {
      #ifdef _YDLE_DEBUG
      ...
      m_initializedState = true;
      writeEEProm();
    } else if (litype==YDLE_CMD_RESET && checkSignal(frame))
    {
      #ifdef _YDLE_DEBUG
      log("Reset received ",litype);
      #endif
      resetNode();
    } else if (litype == YDLE_CMD_ON && checkSignal(frame))
    {
      #ifdef _YDLE_DEBUG
      log("ON");
      #endif
    } else if (litype==YDLE_CMD_OFF && checkSignal(frame))
    {
      #ifdef _YDLE_DEBUG
      log("OFF");
      #endif
